...
One tool that can be used to help generate a lot of realistic randomized data is the Faker libraryLinks to an external site.. This library is already included in the Pipfile for this application, so we can try it out. Run python debug.py, and try out some Faker methods:
...
fake.name()
# => 'Christopher Ortega'
As you can see, every time we call the name method, we get a new random name. Faker has a lot of built-in randomized data generatorsLinks to an external site. that you can use:
...
Let's use Faker to generate 50 random games (we will use the random library to generate prices). Replace the data after our data deletion in the seed.py file with the following code:
